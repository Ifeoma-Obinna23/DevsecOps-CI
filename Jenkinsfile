...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'main', url: 'https://github.com/Ifeoma-Obinna23/DevsecOps-CI.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit ifeobi/reddit:latest "
                       sh "docker push ifeobi/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image ifeobi/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.142.76.235 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.137.188.9:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#cicd-alert', //update and provide your channel name
